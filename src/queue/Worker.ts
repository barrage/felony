...
import FelonyJobDispatched from "../../support/events/FelonyJobDispatched";
...
export default class Worker {
   ...
   * @private
   */
  static _client: Redis | null = null;
  ...
  async listen(queue: string): Promise<void> {
    ...
    while (!this.felony.shuttingDown) {
      ...
      if (job && job.__kind === "Job") {
        ...
        if (!job.finishedAt) {
          await this.push(job, `failed:${queue}`);
   ...
   * @return {Promise<void>}
   */
  async stop(force: number = 300): Promise<void> {
   ...
   * @return {Promise<Job>}
   */
  async getJob(job: string, payload: any = {}): Promise<Job | null> {
    let loaded = null;
    ...
    for (const Imported of this.jobs) {
      ...
      loaded = new Imported(payload, this.felony);
   ...
   * @return {Promise<Job>}
   */
  async dispatch(job: string, payload: any = {}, queue: string | null = null): Promise<Job> {
    const loaded = await this.getJob(job, payload);

    if (!loaded || loaded.__kind !== "Job") {
   ...
   * }>}
   */
  async batchDispatch(jobs: {job: string, payload: any, queue: string | boolean}[]): Promise<{
   ...
   */
  async pop(queue: string): Promise<Job | void> {
   ...
   * @return {Promise<Job>}
   */
  async push(job: Job, queue?: string): Promise<Job> {
