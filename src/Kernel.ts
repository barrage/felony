...
export default class Kernel {
  ...
  async config(): Promise<object> {
    ...
    for (const file of files) {
      ...
      const data: any = await Kernel.loadConfig(file);
   ...
   */
  async readRecursive(dir: string, suffix: string[] = [".js"], dirIgnore: string[] = []): Promise<string[]> {
   ...
   * @private
   */
  static async loadConfig(filePath: string): Promise<object> {
    try {
