...
import FelonyServerReady from "../../support/events/FelonyServerReady";
import FelonyServerLoaded from "../../support/events/FelonyServerLoaded";
import FelonyServerListening from "../../support/events/FelonyServerListening";
import FelonyServerBeforeRouteCompile from "../../support/events/FelonyServerBeforeRouteCompile";
...
export default class Server {
   ...
   * @return {Promise<void>}
   */
  close(force: number = 300): Promise<void> {
   ...
   */
  async compileCallbacks(route: RouteInterface): Promise<Function[]> {
    ...
    const callbacks = (await route.getMiddleware())
      .map((middleware) => async (request: Express.Request, response: Express.Response, next: Function) => {
    ...
    callbacks.push(async (request: Express.Request, response: Express.Response) => {
  ...
  async setupHelmet(): Promise<void> {
    ...
    const H: any = this.kernel.felony.config.server.helmet;
  ...
  async setupCors(): Promise<void> {
    ...
    const H: any = this.kernel.felony.config.cors;
